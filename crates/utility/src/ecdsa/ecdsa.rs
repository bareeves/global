...
pub fn generate_keypair(secret_key_bytes: &[u8]) -> Result<EcdsaKeyPair, EcdsaKeyPairError> {
    let secp = Secp256k1::new();
    let sk = SecretKey::from_slice(secret_key_bytes)
        .map_err(|e| EcdsaKeyPairError::SecretKeyParseError(e.to_string()))?;
...
pub fn sign_messagehash(kp: &EcdsaKeyPair, message_hash: Hash) -> Result<Vec<u8>, EcdsaKeyPairError> {
    let secp = Secp256k1::new();
    let message = Message::from_digest_slice(&message_hash.as_bytes())
        .map_err(|e| EcdsaKeyPairError::MessageCreationError(e.to_string()))?;
...
) -> Result<bool, EcdsaKeyPairError> {
    let secp = Secp256k1::new();
    let public_key = PublicKey::from_slice(public_key_bytes)
        .map_err(|e| EcdsaKeyPairError::PublicKeyParseError(e.to_string()))?;
    let signature = Signature::from_compact(compact_signature)
        .map_err(|e| EcdsaKeyPairError::SignatureCreationError(e.to_string()))?;
    let message = Message::from_digest_slice(&message_hash.as_bytes())
        .map_err(|e| EcdsaKeyPairError::MessageCreationError(e.to_string()))?;
